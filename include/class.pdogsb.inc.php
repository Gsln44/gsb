...
class PdoGsb{   		
	private static $serveur='mysql:host=172.21.105.1';
      	...
      	private static $bdd='dbname=gsbDylan';   		
...
*/
	...
	public function getLesVisiteursDanslaMemeZone($idVisiteur){
		...
		return $lesZonesVisiteurs;

	}
	public function getLesMemeZoneDelegues($idVisiteur){
		//$req = "select nom,prenom from role,visiteur where role.id=2 and idRole=2 and visiteur.idRole = role.id and visiteur.id ='$idVisiteur'";
		$req = "select idVisiteur, nom,prenom from affecter,visiteur where visiteur.id=affecter.idVisiteur and idRole=2 and idZone =(select idZone from affecter where idVisiteur='$idVisiteur')";
		$res = PdoGsb::$monPdo->query($req);
		$lesDelegues =array();
		$laLigne = $res->fetch();
		while($laLigne != null)	{
	...
	public function creeNouvelleZones($libelle){
		$req = "insert into zone
		values('','$libelle')";
		PdoGsb::$monPdo->exec($req);
	}
